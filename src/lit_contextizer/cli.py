...
def main(out_dir, networks_dir, paper_subset, insider_context_type, parse_files, dump_annots_context, load_max, min_conf):
    """Run main function."""
    ...
    if parse_files:
        dl.parse_pubmed_full_texts(load_max=load_max)
        print("Done with full texts")
        # dl.all_pubmed_relations_df.to_csv(os.path.join(out_dir, "all_pubmed_relations_df.csv"), index=False)
        dl.all_pubmed_relations_df.to_csv(os.path.join(out_dir, "all_pubmed_relations_citationDist_v2.tsv"), sep='\t', index=False)
    ...
    if paper_subset == "insider":
        ...
        # dl.output_section_names(relation_subset, out_dir)  # output the names of the sections to create normalizing list
        features_df = dl.extract_features_from_all_pubmed_ppis(relation_subset, context_type=insider_context_type)

        context_type_filename = 'combined' if insider_context_type is None else insider_context_type
        features_df.to_csv(os.path.join(out_dir, f"{context_type_filename}_insider_papers_features_df.tsv"), index=False, sep='\t')
        ...
        context_list = ['adipose_tissue', 'cardiac_muscle', 'liver', 'lung']
        context_lookup_list = ['adipo', 'cardi', 'liver', 'lung']

        for context, context_lookup in zip(context_list, context_lookup_list):
            topnet_file = networks_dir + "top_network/" + context + "_top"
        ...
        cardiac_res_path = os.path.join(out_dir, f"cardiac_muscle_ppi_pmc_df_conf{min_conf}.csv")
        ...
        all_res_combined = pd.concat([adipose_res, cardiac_res, liver_res, lung_res], ignore_index=True)
